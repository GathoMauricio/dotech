...
class CompanyController extends Controller
{
    public function index()
    {
        ...
        //$clientes = Company::where('status', 'Cliente')->paginate(10);
        //$prospectos = Company::where('status', 'Prospecto')->paginate(10);
    ...
    public function indexAjax()
    {
        ...
        foreach ($companies as $company) {
            ...
            $spanDocuments = "<a href='" . route('company_documents', $company->id) . "' style='cursor:pointer;color:blue;'><span title='Documentación...' class='icon icon-file-pdf'></span> Documentación</a>";
    ...
    {
        $origenes = ClienteOrigen::all();
        return view('companies.create', compact('origenes'));
    }
    ...
    public function storeOrigen(Request $request)
    {
        ClienteOrigen::create([
            'author_id' => \Auth::user()->id,
            'origen' => $request->origen
        ]);
        return redirect()->back()->with('message', 'El origen se agregó con exito.');
    }
}
