        ...
        <tbody id="tbl_projects_to_search">
            @if (strlen($search) > 0)
                ...
                @foreach ($whitdrawals as $whitdrawal)
                    ...
                    <td>{{ $whitdrawal->ID }}</td>
                    <td>
                        <a href="{{ route('proyecto.show', $whitdrawal->ID_VENTA) }}" target="_blank">
                            {{ $whitdrawal->FOLIO }}
                    ...
                    <td>
                        <center>
                            ...
                            @if ($whitdrawal->FACTURA == 'SI')
                                ...
                                @endif
                                <br>
                                <span wire:click = "validarFactura({{ $whitdrawal->ID }})" class = "icon-spinner11"
                                    style = "cursor:pointer;color:#3498DB;"></span>
                            @else
                                N/A
                            @endif
                        </center>
                    ...
                    <td><a href="javascript:void(0)"
                            wire:click="showTransactions({{ $whitdrawal->ID }})">{{ count(App\Whitdrawal::find($whitdrawal->ID)->transactions) }}
                            Registros<a></td>
                    @if (Auth::user()->rol_user_id == 1)
                        <td>
                            <a href="javascript:void(0)" onclick="aproveWithdrawalModal({{ $whitdrawal->ID }});"><span
                                    class="icon-point-up" title="Aprovar" style="cursor:pointer;color:#74DF00">
                                    Aprobar</span></a>
                            <br>
                            <a href="javascript:void(0)" onclick="disaproveWithdrawal({{ $whitdrawal->ID }});"><span
                                    class="icon-point-down" title="Desaprobar" style="cursor:pointer;color:#FFBF00">
                                    Rechazar</span></a>
                            <br>
                            <a href="javascript:void(0)" wire:click="edit({{ $whitdrawal->ID }});"><span
                                    class="icon-pencil" title="Desaprobar" style="cursor:pointer;color:#ff9100">
                                    Editar</span></a>
                            <br>
                            <a href="javascript:void(0)" onclick="deleteWithdrawal({{ $whitdrawal->ID }});"><span
                                    class="icon-bin" title="Eliminar" style="cursor:pointer;color:#DF0101">
                                    Eliminar</span></a>
                            <br>
                            @if ($whitdrawal->FACTURA == 'SI')
                                @if (!empty($whitdrawal->DOCUMENTO))
                                    <a href="{{ env('APP_URL') . '/storage/' . $whitdrawal->DOCUMENTO }}"
                                        target="_BLANK"><span class="icon-eye"></span> Ver</a>
                                @else
                                    <a href="#" onclick="addWhitdralDocumentModal({{ $whitdrawal->ID }});"><span
                                            class="icon-upload"></span> Cargar</a>
                                @endif
                            @else
                                N/A
                            @endif
            ...
            @else
                @foreach ($whitdrawals as $whitdrawal)
                    ...
                    <td>{{ $whitdrawal->id }}</td>
                    <td>
                        <a href="{{ route('proyecto.show', $whitdrawal->sale->id) }}" target="_blank">
                            {{ $whitdrawal->sale['folio_proyecto'] }}
                    ...
                    <td><a href="javascript:void(0)"
                            wire:click="showTransactions({{ $whitdrawal->id }})">{{ count($whitdrawal->transactions) }}
                            Registros</a></td>
                    @if (Auth::user()->rol_user_id == 1)
                        <td>
                            <a href="javascript:void(0)" onclick="aproveWithdrawalModal({{ $whitdrawal->id }});"><span
                            ...
                            @if ($whitdrawal->invoive == 'SI')
                                @if (!empty($whitdrawal->document))
                                    <a href="{{ env('APP_URL') . '/storage/' . $whitdrawal->document }}"
                                        target="_BLANK"><span class="icon-eye"></span> Ver</a>
                                @else
                                    <a href="#"
                                        onclick="addWhitdralDocumentModal({{ $whitdrawal->id }});"><span
                        ...
                        @if ($whitdrawal->invoive == 'SI')
                            @if (!empty($whitdrawal->document))
                                <td class="text-center">
                                    ...
                                    <br>
                                    <a href="javascript:void(0)" wire:click="edit({{ $whitdrawal->id }});"><span
                                            class="icon-pencil" title="Desaprobar"
                                            style="cursor:pointer;color:#ff9100"> Editar</span></a>
                                </td>
                            @else
                                <td class="text-center">
                                    <a href="javascript:void(0)"
