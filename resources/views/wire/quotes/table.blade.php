...
-->
<a href="#" onclick="createQuote();"><span class="icon-plus"></span> Agregar cotización <img
        src="{{ asset('img/wire.png') }}" height="16"></a>
    ...
    <table class="table table-bordered">
        ...
        <tbody>
            @foreach ($quotes as $quote)
                @if (strlen($search) > 0)
                    <tr>
                        ...
                        <td>
                            <a href="#"
                                onclick="sendQuoteModal({{ $quote->ID }},'{{ $quote->EMAIL }}');"><span
                                    class="icon-envelop" title="Enviar" style="cursor:pointer;color:#D7DF01">
                                    Enviar</span></a>
                            <br>
                            <a href="javascript:void(0);" onclick="loadPdf({{ $quote->ID }})"><span
                                    class="icon-file-pdf" title="Productos" style="cursor:pointer;color:red">
                                    PDF</span></a>
                            <br>
                            <a href="javascript:void(0);"
                                wire:click.prevent="showFullModalProducts({{ $quote->ID }});"><span class="icon-eye"
                                    title="Productos" style="cursor:pointer;color:#3498DB"> Productos</span></a>
                            <br>
                            <a href="#" onclick="changeStatusModal({{ $quote->ID }});"><span
                                    class="icon-checkmark" title="Cambiar estatus" style="cursor:pointer;color:#2ECC71">
                                    Estatus</span></a>
                            <br>
                            <a href="#" onclick="editQuote({{ $quote->ID }});"><span class="icon-pencil"
                                    title="Editar" style="cursor:pointer;color:#F39C12"> Editar</span></a>
                            <br>
                            <a onclick="saleFollowModal({{ $quote->ID }});" href="#"><span class="icon-bubble"
                                    title="Seguimientos" style="cursor:pointer;color:#2980B9"> Seguimientos</span></a>
                            @if (Auth::user()->rol_user_id == 1)
                                <br>
                                <a href="#" onclick="deleteSale({{ $quote->ID }})"><span class="icon-bin"
            ...
            -->
                        ...
                        <td>
                            <a href="#"
                                onclick="sendQuoteModal({{ $quote->id }},'{{ $quote->department['email'] }}');"><span
                                    class="icon-envelop" title="Enviar" style="cursor:pointer;color:#D7DF01">
                                    Enviar</span></a>
                            <br>
                            <a href="javascript:void(0);" onclick="loadPdf({{ $quote->id }})"><span
