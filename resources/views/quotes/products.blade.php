...
@section('content')
    <h4>Productos de
        <a href="{{ route('proyecto.show', $sale->id) }}">{{ $sale->description }}</a> para
        <a href="{{ route('clientes.show', $sale->cliente->id) }}">{{ $sale->company['name'] }}</a>
    </h4>
    ...
    &nbsp;&nbsp;
    <a href="#" onclick="sendQuoteModal({{ $sale->id }},'{{ $sale->department['email'] }}')"><span
            class="icon-envelop"></span> Enviar cotización</a>
    @if (count($products) <= 0)
