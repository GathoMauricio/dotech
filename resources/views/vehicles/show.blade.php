...
@section('content')
    ...
    <table style="width:100%;">
        <tr>
            <th colspan="5" class="text-center font-weight-bold" style="background-color:#d30035;color:white;">Informació
                general</th>
        </tr>
        <tr>
    ...
    <table style="width:100%;">
        <tr>
            <td onclick="showVehicleTab('fotos');" class="text-center font-weight-bold"
                style="background-color:#d30035;color:white;cursor:pointer;">
                Fotos
            </td>
            <td onclick="showVehicleTab('mantenimientos');" class="text-center font-weight-bold"
                style="background-color:#d30035;color:white;cursor:pointer;">
                Mantenimientos
            </td>
            <td onclick="showVehicleTab('salidas');" class="text-center font-weight-bold"
                style="background-color:#d30035;color:white;cursor:pointer;">
                Salidas
            </td>
            <td onclick="showVehicleTab('verificaciones');" class="text-center font-weight-bold"
                style="background-color:#d30035;color:white;cursor:pointer;">
                Verificaciones
            </td>
            @if (Auth::user()->rol_user_id == 1 || Auth::user()->rol_user_id == 2)
                <td onclick="showVehicleTab('documentacion');" class="text-center font-weight-bold"
                    style="background-color:#d30035;color:white;cursor:pointer;">
                    Documentación
                </td>
            @endif
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    <th colspan="3" class="text-center" style="background-color:#d30035;color:white;">
                        <a href="#" onclick="addVehicleImage({{ $vehicle->id }})" title="Añadir" class="float-right"
                            style="color:white;">[ <span class="icon-upload"></span> ]</a>
                        Fotos
                    </th>
                </tr>
                <tr>
            ...
            <tbody>
                @foreach ($vehicleImages as $vehicleImage)
                    <tr>
                        <td><a href="{{ asset('storage') }}/{{ $vehicleImage->image }}" target="_blank"><img
                                    src="{{ asset('storage') }}/{{ $vehicleImage->image }}" width="120" /></a></td>
                        <td>{{ $vehicleImage->description }}</td>
                        @if (Auth::user()->rol_user_id == 1)
                            <td>
                                <a href="#" onclick="deleteVehicleImage({{ $vehicleImage->id }})"><span
                                        class="icon-bin" title="Eliminar..." style="cursor:pointer;color:#E74C3C">
                                        Eliminar</span></a>
                            </td>
                        @endif
                    </tr>
        ...
        <table class="table table-striped">
            <thead>
                <tr>
                    <th colspan="7" class="text-center" style="background-color:#d30035;color:white;">
                        <a href="#" onclick="addMaintenanceVehicle({{ $vehicle->id }})" title="Añadir"
                            class="float-right" style="color:white;">[ <span class="icon-upload"></span> ]</a>
                        Manteniemientos
                    </th>
                </tr>
                <tr>
            ...
            <tbody>
                @foreach ($maintenances as $maintenance)
                    <tr>
                        <td>{{ $maintenance->author['name'] }} {{ $maintenance->author['middle_name'] }}
                            {{ $maintenance->author['last_name'] }}</td>
                        ...
                        <td>
                            @if (is_numeric($maintenance->amount))
                        ...
                        <td>
                            <a href="{{ route('maintenance_show', $maintenance->id) }}"><span class="icon-eye"
                                    title="Ver..." style="cursor:pointer;color:#2E86C1"> Ver</span></a>
                            <br />
                            <a href="{{ route('maintenance_edit', $maintenance->id) }}"><span class="icon-pencil"
                                    title="Editar..." style="cursor:pointer;color:#EB984E"> Editar</span></a>
                            <br />
                            @if (Auth::user()->rol_user_id == 1)
                                <a href="#" onclick="deleteMaintenance({{ $maintenance->id }})"><span
                                        class="icon-bin" title="Eliminar..." style="cursor:pointer;color:#E74C3C">
                                        Eliminar</span></a>
                            @endif
                        </td>
            ...
            <tbody>
                @foreach ($vehicleHistories as $vehicleHistory)
                    <tr>
                        ...
                        <td>{{ $vehicleHistory->author['name'] }} {{ $vehicleHistory->author['middle_name'] }}
                            {{ $vehicleHistory->author['last_name'] }}</td>
                        ...
                        <td>
                            <a href="{{ route('vehicle_history_show', $vehicleHistory->id) }}"><span class="icon-eye"
                                    title="Ver..." style="cursor:pointer;color:#2E86C1"> Ver</span></a>
                            <br />
                            <!--
            ...
            <thead>
                <tr>
                    <th colspan="4" class="text-center" style="background-color:#d30035;color:white;">
                        <a href="#" onclick="addVehicleVerification({{ $vehicle->id }})" title="Añadir"
                            class="float-right" style="color:white;">[ <span class="icon-upload"></span> ]</a>
                        Verificaciones
                    </th>
                </tr>
            ...
            <tbody>
                @foreach ($verifications as $verification)
                    <tr>
                        ...
                        <td class="text-center"><a href="{{ asset('storage') }}/{{ $verification->image }}"
                                target="_blank"><img src="{{ asset('storage') }}/{{ $verification->image }}"
                                    width="200" /></a></td>
                    </tr>
                @endforeach
    ...
    <div id="documentacion_vehicles_container_tab" style="display:none;">
        <table class="table table-striped">
            <thead>
                <tr>
                    <th colspan="3" class="text-center" style="background-color:#d30035;color:white;">
                        <a href="#" onclick="addVehicleDocument({{ $vehicle->id }})" title="Añadir"
                            class="float-right" style="color:white;">[ <span class="icon-upload"></span> ]</a>
                        Documentación
                    </th>
                </tr>
            ...
            <tbody>
                @foreach ($documents as $document)
                    <tr>
                        ...
                        <td><a href="{{ asset('storage') }}/{{ $document->file }}" target="_blank">Ver documento</a>
                        </td>
                    </tr>
                @endforeach
            ...
            <tbody>
                @foreach ($inventarios as $inventario)
                    <tr>
                        ...
                        <td>{{ $inventario->autor->name }} {{ $inventario->autor->middle_name }}
                            {{ $inventario->autor->last_name }}</td>
