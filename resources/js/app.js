    ...
    $.ajax({
        ...
        success: (data) => {
            ...
            $.each(data, (index, item) => {
                ...
                ${
                    item.paid == "SI"
                        ? '<tr class="bg-info"  id="tr_whitdrawal_' +
                          item.id +
                          '">'
                        : '<tr id="tr_whitdrawal_' + item.id + '">'
                }
                <td>${item.id}</td>
                <!--<td>${item.provider}</td>-->
                <!--<td>${item.company}</td>-->
        ...
        select: (event, ui) => {
            ...
            $.ajax({
                ...
                success: (data) => {
                    ...
                    $("#span_binnacle_company_show_modal").text(
                        data.company.name
                    );
                    $("#span_binnacle_project_show_modal").text(
                        data.sale.description
                    );
                    $("#span_binnacle_author_show_modal").text(
                        data.author.name +
                            " " +
                            data.author.middle_name +
                            " " +
                            data.author.last_name
                    );
                    $("#span_binnacle_description_show_modal").text(
                        data.binnacle.description
                    );
                    $("#span_binnacle_date_show_modal").text(
                        data.binnacle.created_at + " Hrs"
                    );
        ...
        select: (event, ui) => {
            ...
            $.ajax({
                ...
                success: (data) => {
                    ...
                    $("#span_whitdrawal_provider_show_modal").text(
                        data.provider
                    );
                    $("#span_whitdrawal_project_show_modal").text(data.project);
                    $("#span_whitdrawal_description_show_modal").text(
                        data.description
                    );
                    $("#span_whitdrawal_author_show_modal").text(data.author);
                    $("#span_whitdrawal_quantity_show_modal").text(
                        data.quantity
                    );
...
window.addQuoteByCompanyModal = (company_id) => {
    ...
    $.ajax({
        ...
        data: { id: company_id },
        success: (data) => {
            $("#cbo_add_quote_by_company_department").html(
...
window.viewStockProductImages = (binnacle_id, count) => {
    if (count > 0) {
        ...
        viewer.permalink = () => {
            window.open(
                show_binnacle_image + "/" + $("#PhotoViewerByline").text()
            );
        };
        //viewer.setOnClickEvent(viewer.permalink);
        $.ajax({
...
Livewire.on("showFullModalProducts", () =>
    $("#full_modal_index_products").css("display", "block")
);
Livewire.on("dissmisCreateProductQuote", () =>
    $("#create_product_quote").modal("hide")
);
Livewire.on("editQuoteProductModal", () => $("#edit_product_quote").modal());
Livewire.on("dissmisEditProductQuote", () =>
    $("#edit_product_quote").modal("hide")
);
...
Livewire.on("dissmisEditWhitdrawalModal", () =>
    $("#edit_whitdrawal_modal").modal("hide")
);

Livewire.on("showTransaccionesModal", () =>
    $("#show_transacciones_modal").modal()
);
...
window.createQuote = () =>
    $("#full_modal_create_quote").css("display", "block");
window.createProductQuote = () => $("#create_product_quote").modal();
window.showFullModalProducts = () =>
    $("#full_modal_index_products").css("display", "block");
